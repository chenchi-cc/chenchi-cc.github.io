...
- **具体工作：**
  ...
  - **任务治理：** 针对PUSH任务数据分层不合理，自助化率低的现状，进行了重构的工作。
    ...
    - 充分利用spark3特性，进行参数性能调优，调整并行度，消除cache滥用，去除逻辑中不等join，全部通过等值join来替代；
...
- **具体工作：**
  ...
  - 百家号数仓数据按照宽表建模，简化了分层和数据域的概念，减少数据分层，底层多为文件，上层主要分为作者和文章两大主题宽表（明细粒度），将多个业务域（百家号、动态、好看、feed）统一，解决了数据孤岛问题，宽表分多版本产出，会将宽表同步CK，方便使用方一站式自助分析。
...
- **难点：**
  - **百家号数仓分版本产出方案：** 以文章宽表为例，最细粒度是nid粒度，整体还是采用“全量同步+增量同步”的模式，共分为5个版本产出不同类型的数据，下游使用方通过对应的版本产出标识配置依赖（可以根据sql字段自动提取对应的版本）。
    - V1是数仓的base表，最先产出，确定量级、唯一主键等，V1产出时非V1版本的其他字段置NULL；
      - V1版本数据，主要是从物料库和正排库透传nid粒度的属性和加工一些核心字段逻辑（比如是否历史可分发，首次发布成功时间等）；
      - 主表（左表）是全量的nid信息，是通过常驻的spark程序每天进行近实时的merge，用以涵盖整个文章表的nid范围以及一些type、category等基础信息；
      - 从表1是物料库表，从表2是正排库表，物料库和正排库都有实时订阅（schema包括nid，update_time，json_value），发生update就会订阅到，然后按照小时聚合->天聚合->merge历史存量->解析物料库字段和正排库字段，有些nid只有物料库信息，有些只有正排库信息，有些都有，需要做对应处理，通过关联从表1和从表2来获取物料库和正排库的透传属性；
      - 从表3是作者表（作者表也是分版本），用以关联作者对应属性，会有头部机构作者发文上千万，需要处理数据倾斜问题；
    - V2-V5按照数据类型划分（带货，后验，审核，收入），均分为两个阶段：单独写HDFS文件和通过主键merge回原表，第一阶段互不影响（谁先依赖就绪谁先运行），第二阶段属于竞争关系，通过抢占锁机制，先获取锁的任务优先merge（非当前版本的字段取表中的原值），没抢到锁会原地等待；
      - V1数据存到base_path/库名/表名/日期/V1目录，然后挂载到hive表分区下；
      - V2-V5分别把各自nid粒度的计算结果存入HDFS，然后merge回原表，merge后的表存到base_path/库名/表名/日期/V2-V5对应目录，每次通过挂载的方式，改变hive表的指向；
    - 每个版本会有一个当前版本字段的配置文件，获取表元数据metadata的全量字段，两者diff就是非当前版本的字段，配置文件会接入数据平台服务，方便配置依赖关系；
    - 多个版本同时merge会出现数据异常，因此通过锁机制解决，锁机制通过 表+分区 粒度进行事务隔离，具体如下：
...
- **效果：**
  - PUSH数仓支持下游工程团队分析和算法团队模型训练，促进了PUSH dau的增长。
  - 百家号数仓需求平均交付时长由4.7d下降为2.8d，数仓存储平均节省了2.6PB，易用性得到极大的提升。
...
- **具体工作：**
  - 采用预聚合的思想，把不同维度按照配置信息进行交叉组合，产出每种交叉维度对当前指标带来的影响贡献度（本期值，上期值，偏移量，波动率，偏移量占比等），从而定位出对当前指标产生影响最大的维度信息。
    - 项目框架参考：[https://github.com/chenchi-cc/data-attribution-analysis-framework](https://github.com/chenchi-cc/data-attribution-analysis-framework)
...
- **效果：**
  - 运营单次进行指标异常归因的耗时，由线下分析4h减少到1h，借助大模型能力，能快速定位出影响指标波动的关键因素。